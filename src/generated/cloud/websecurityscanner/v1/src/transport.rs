...
impl super::stub::WebSecurityScanner for WebSecurityScanner {
    ...
    async fn delete_scan_config(
        ...
        .or_else(|| {
            let path = format!(
                "/v1/{}",
                try_match(Some(&req).map(|m| &m.name).map(|s| s.as_str()), &[Segment::Literal("projects/"), Segment::SingleWildcard, Segment::Literal("/scanConfigs/"), Segment::SingleWildcard])?,
            );
        ...
        .ok_or_else(|| {
            let mut paths = Vec::new();
            {
                ...
                let builder = builder.maybe_add(
                    ...
                    &[Segment::Literal("projects/"), Segment::SingleWildcard, Segment::Literal("/scanConfigs/"), Segment::SingleWildcard],
                    "name",
    ...
    async fn update_scan_config(
        ...
        .or_else(|| {
            let path = format!(
                "/v1/{}",
                try_match(Some(&req).and_then(|m| m.scan_config.as_ref()).map(|m| &m.name).map(|s| s.as_str()), &[Segment::Literal("projects/"), Segment::SingleWildcard, Segment::Literal("/scanConfigs/"), Segment::SingleWildcard])?,
            );
        ...
        .ok_or_else(|| {
            let mut paths = Vec::new();
            {
                let builder = PathMismatchBuilder::default();
                let builder = builder.maybe_add(
                    Some(&req).and_then(|m| m.scan_config.as_ref()).map(|m| &m.name).map(|s| s.as_str()),
                    &[Segment::Literal("projects/"), Segment::SingleWildcard, Segment::Literal("/scanConfigs/"), Segment::SingleWildcard],
                    "scan_config.name",
                    "projects/*/scanConfigs/*");
                paths.push(builder.build());
    ...
    async fn get_scan_run(
        ...
        .or_else(|| {
            let path = format!(
                "/v1/{}",
                try_match(Some(&req).map(|m| &m.name).map(|s| s.as_str()), &[Segment::Literal("projects/"), Segment::SingleWildcard, Segment::Literal("/scanConfigs/"), Segment::SingleWildcard, Segment::Literal("/scanRuns/"), Segment::SingleWildcard])?,
            );
            let path_template = "/v1/{name}";
        ...
        .ok_or_else(|| {
            let mut paths = Vec::new();
            {
                ...
                let builder = builder.maybe_add(
                    Some(&req).map(|m| &m.name).map(|s| s.as_str()),
                    &[Segment::Literal("projects/"), Segment::SingleWildcard, Segment::Literal("/scanConfigs/"), Segment::SingleWildcard, Segment::Literal("/scanRuns/"), Segment::SingleWildcard],
                    "name",
    ...
    async fn list_scan_runs(
        ...
        .or_else(|| {
            let path = format!(
                "/v1/{}/scanRuns",
                try_match(Some(&req).map(|m| &m.parent).map(|s| s.as_str()), &[Segment::Literal("projects/"), Segment::SingleWildcard, Segment::Literal("/scanConfigs/"), Segment::SingleWildcard])?,
            );
    ...
    async fn list_crawled_urls(
        ...
        .or_else(|| {
            let path = format!(
                "/v1/{}/crawledUrls",
                try_match(Some(&req).map(|m| &m.parent).map(|s| s.as_str()), &[Segment::Literal("projects/"), Segment::SingleWildcard, Segment::Literal("/scanConfigs/"), Segment::SingleWildcard, Segment::Literal("/scanRuns/"), Segment::SingleWildcard])?,
            );
    ...
    async fn get_finding(
        ...
        .or_else(|| {
            let path = format!(
                "/v1/{}",
                try_match(Some(&req).map(|m| &m.name).map(|s| s.as_str()), &[Segment::Literal("projects/"), Segment::SingleWildcard, Segment::Literal("/scanConfigs/"), Segment::SingleWildcard, Segment::Literal("/scanRuns/"), Segment::SingleWildcard, Segment::Literal("/findings/"), Segment::SingleWildcard])?,
            );
            let path_template = "/v1/{name}";
        ...
        .ok_or_else(|| {
            let mut paths = Vec::new();
            {
                ...
                let builder = builder.maybe_add(
                    Some(&req).map(|m| &m.name).map(|s| s.as_str()),
                    &[Segment::Literal("projects/"), Segment::SingleWildcard, Segment::Literal("/scanConfigs/"), Segment::SingleWildcard, Segment::Literal("/scanRuns/"), Segment::SingleWildcard, Segment::Literal("/findings/"), Segment::SingleWildcard],
                    "name",
