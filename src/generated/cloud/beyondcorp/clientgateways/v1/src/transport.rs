...
impl super::stub::ClientGatewaysService for ClientGatewaysService {
    async fn list_client_gateways(
        ...
        .or_else(|| {
            ...
            let path_template = "/v1/{parent}/clientGateways";
    ...
    async fn get_client_gateway(
        ...
        .or_else(|| {
            let path = format!(
                "/v1/{}",
                try_match(Some(&req).map(|m| &m.name).map(|s| s.as_str()), &[Segment::Literal("projects/"), Segment::SingleWildcard, Segment::Literal("/locations/"), Segment::SingleWildcard, Segment::Literal("/clientGateways/"), Segment::SingleWildcard])?,
            );
    ...
    async fn create_client_gateway(
        ...
        let builder = builder
                .query(&[("$alt", "json;enum-encoding=int")])
                .header("x-goog-api-client", reqwest::header::HeaderValue::from_static(&info::X_GOOG_API_CLIENT_HEADER));
        let body = gaxi::http::handle_empty(req.client_gateway, &method);
        self.inner.execute(
            builder,
            body,
            options,
