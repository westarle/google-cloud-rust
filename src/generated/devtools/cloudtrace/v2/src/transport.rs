...
impl super::stub::TraceService for TraceService {
    async fn batch_write_spans(
        ...
        let options = {
            ...
            if let Some(rn) = resource_name {
                let full_resource_name = format!("//cloudtrace.googleapis.com/{}", rn);
    ...
    async fn create_span(
        ...
        .or_else(|| {
            let path = format!(
                "/v2/{}",
                try_match(Some(&req).map(|m| &m.name).map(|s| s.as_str()), &[Segment::Literal("projects/"), Segment::SingleWildcard, Segment::Literal("/traces/"), Segment::SingleWildcard, Segment::Literal("/spans/"), Segment::SingleWildcard])?,
            );
        ...
        .ok_or_else(|| {
            let mut paths = Vec::new();
            {
                ...
                let builder = builder.maybe_add(
                    Some(&req).map(|m| &m.name).map(|s| s.as_str()),
                    &[Segment::Literal("projects/"), Segment::SingleWildcard, Segment::Literal("/traces/"), Segment::SingleWildcard, Segment::Literal("/spans/"), Segment::SingleWildcard],
                    "name",
