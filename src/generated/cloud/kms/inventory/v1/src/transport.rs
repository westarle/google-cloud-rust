...
impl super::stub::KeyTrackingService for KeyTrackingService {
    async fn get_protected_resources_summary(
        ...
        .or_else(|| {
            ...
            let path_template = "/v1/{name}/protectedResourcesSummary";
    ...
    async fn search_protected_resources(
        ...
        .or_else(|| {
            let path = format!(
                ...
                try_match(Some(&req).map(|m| &m.scope).map(|s| s.as_str()), &[Segment::Literal("organizations/"), Segment::SingleWildcard])?,
            );
            let path_template = "/v1/{scope}/protectedResources:search";
