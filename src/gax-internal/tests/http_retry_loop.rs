...
mod tests {
    ...
    async fn retry_loop_retry_success(enable_tracing: bool) -> Result<()> {
        ...
        let test_id = if enable_tracing {
            "retry_loop_retry_success_tracing_on"
        } else {
            "retry_loop_retry_success_tracing_off"
        };
        ...
        let spans = TestLayer::capture(test_id);
        if enable_tracing {
            assert_eq!(spans.len(), 3, "Expected 3 spans");
            // Span 1 (attempt 0)
            assert_eq!(spans[0].name, "http_request_attempt");
            assert!(!spans[0].attributes.contains_key(semconv::trace::HTTP_REQUEST_RESEND_COUNT));
            // Span 2 (attempt 1)
            assert_eq!(spans[1].name, "http_request_attempt");
            assert_eq!(spans[1].attributes.get(semconv::trace::HTTP_REQUEST_RESEND_COUNT), Some(&"1".to_string()));
            // Span 3 (attempt 2)
            assert_eq!(spans[2].name, "http_request_attempt");
            assert_eq!(spans[2].attributes.get(semconv::trace::HTTP_REQUEST_RESEND_COUNT), Some(&"2".to_string()));
            // TODO: Assert other attributes for retries and final success
        } else {
    ...
    async fn retry_loop_too_many_transients(enable_tracing: bool) -> Result<()> {
        ...
        let test_id = if enable_tracing {
            "retry_loop_too_many_transients_tracing_on"
        } else {
            "retry_loop_too_many_transients_tracing_off"
        };
        ...
        if enable_tracing {
            ...
            assert_eq!(spans[2].name, "http_request_attempt");
            assert_eq!(spans[2].attributes.get(semconv::trace::HTTP_REQUEST_RESEND_COUNT), Some(&"2".to_string()));
            // TODO: Assert attributes for transient failures and retries
        } else {
            assert_eq!(spans.len(), 0, "Expected 0 spans when tracing is disabled");
        }
        Ok(())
