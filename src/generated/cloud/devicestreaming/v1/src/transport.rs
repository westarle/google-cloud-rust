...
impl super::stub::DirectAccessService for DirectAccessService {
    async fn create_device_session(
        ...
        let options = {
            ...
            if let Some(rn) = resource_name {
                let full_resource_name = format!("//devicestreaming.googleapis.com/{}", rn);
    ...
    async fn get_device_session(
        ...
        .or_else(|| {
            let path = format!(
                "/v1/{}",
                try_match(Some(&req).map(|m| &m.name).map(|s| s.as_str()), &[Segment::Literal("projects/"), Segment::SingleWildcard, Segment::Literal("/deviceSessions/"), Segment::SingleWildcard])?,
            );
        ...
        .ok_or_else(|| {
            let mut paths = Vec::new();
            {
                ...
                let builder = builder.maybe_add(
                    ...
                    &[Segment::Literal("projects/"), Segment::SingleWildcard, Segment::Literal("/deviceSessions/"), Segment::SingleWildcard],
                    "name",
    ...
    async fn update_device_session(
        ...
        .or_else(|| {
            let path = format!(
                "/v1/{}",
                try_match(Some(&req).and_then(|m| m.device_session.as_ref()).map(|m| &m.name).map(|s| s.as_str()), &[Segment::Literal("projects/"), Segment::SingleWildcard, Segment::Literal("/deviceSessions/"), Segment::SingleWildcard])?,
            );
        ...
        .ok_or_else(|| {
            let mut paths = Vec::new();
            {
                let builder = PathMismatchBuilder::default();
                let builder = builder.maybe_add(
                    Some(&req).and_then(|m| m.device_session.as_ref()).map(|m| &m.name).map(|s| s.as_str()),
                    &[Segment::Literal("projects/"), Segment::SingleWildcard, Segment::Literal("/deviceSessions/"), Segment::SingleWildcard],
                    "device_session.name",
                    "projects/*/deviceSessions/*");
                paths.push(builder.build());
