...
async fn client_library_operations() -> anyhow::Result<()> {
    let project = std::env::var("GOOGLE_CLOUD_PROJECT")
        .unwrap_or_else(|_| "westarle-78rabs".to_string());
    ...
    let db_path = format!("projects/{}/instances/{}/databases/{}", project, instance, db_id);
    ...
    use google_cloud_spanner::model::{BeginTransactionRequest, TransactionOptions, transaction_options};
    let mut req = BeginTransactionRequest::new();
    req.session = db_client.session.name.clone();
    
    let mut options = TransactionOptions::new();
    options.mode = Some(transaction_options::Mode::ReadOnly(Box::new(transaction_options::ReadOnly::default())));
    req.options = Some(options);
    
    let _ = db_client.spanner.begin_transaction(req, google_cloud_gax::options::RequestOptions::default()).await;
