...
impl super::stub::AppGatewaysService for AppGatewaysService {
    async fn list_app_gateways(
        ...
        .or_else(|| {
            ...
            let path_template = "/v1/{parent}/appGateways";
    ...
    async fn get_app_gateway(
        ...
        .or_else(|| {
            let path = format!(
                "/v1/{}",
                try_match(Some(&req).map(|m| &m.name).map(|s| s.as_str()), &[Segment::Literal("projects/"), Segment::SingleWildcard, Segment::Literal("/locations/"), Segment::SingleWildcard, Segment::Literal("/appGateways/"), Segment::SingleWildcard])?,
            );
    ...
    async fn create_app_gateway(
        ...
        let builder = builder
                .query(&[("$alt", "json;enum-encoding=int")])
                .header("x-goog-api-client", reqwest::header::HeaderValue::from_static(&info::X_GOOG_API_CLIENT_HEADER));
        let body = gaxi::http::handle_empty(req.app_gateway, &method);
        self.inner.execute(
            builder,
            body,
            options,
