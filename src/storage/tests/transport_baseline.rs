...
impl CredentialsProvider for TestCredentialsProvider {
    ...
    ) -> std::result::Result<CacheableResource<HeaderMap>, auth::errors::CredentialsError> {
...
impl StreamingSource for TestStreamingSource {
    ...
    async fn size_hint(&self) -> std::result::Result<SizeHint, std::io::Error> {
        Ok(SizeHint::with_exact(self.data.len() as u64))
    }
}
...
async fn test_error_handling_404() -> Result {
    ...
    let client = create_test_client(&server).await?;
    let _err: google_cloud_storage::Error = client
        .read_object("projects/_/buckets/test-bucket", "test-object")
        .send()
        .await
