...
impl super::stub::FunctionService for FunctionService {
    async fn get_function(
        ...
        .or_else(|| {
            let path = format!(
                "/v2/{}",
                try_match(Some(&req).map(|m| &m.name).map(|s| s.as_str()), &[Segment::Literal("projects/"), Segment::SingleWildcard, Segment::Literal("/locations/"), Segment::SingleWildcard, Segment::Literal("/functions/"), Segment::SingleWildcard])?,
            );
        ...
        .ok_or_else(|| {
            let mut paths = Vec::new();
            {
                ...
                let builder = builder.maybe_add(
                    ...
                    &[Segment::Literal("projects/"), Segment::SingleWildcard, Segment::Literal("/locations/"), Segment::SingleWildcard, Segment::Literal("/functions/"), Segment::SingleWildcard],
                    "name",
        ...
        let options = {
            ...
            if let Some(rn) = resource_name {
                let full_resource_name = format!("//cloudfunctions.googleapis.com/{}", rn);
    ...
    async fn create_function(
        ...
        let options = {
            let resource_name = Option::<&String>::None
                .or(Some(&req.parent))
                                .or(req.function.as_ref().map(|s| &s.kms_key_name))
                ;
            if let Some(rn) = resource_name {
                let full_resource_name = format!("//cloudfunctions.googleapis.com/{}", rn);
                gax::options::internal::set_resource_name(options, full_resource_name)
    ...
    async fn update_function(
        ...
        .or_else(|| {
            let path = format!(
                "/v2/{}",
                try_match(Some(&req).and_then(|m| m.function.as_ref()).map(|m| &m.name).map(|s| s.as_str()), &[Segment::Literal("projects/"), Segment::SingleWildcard, Segment::Literal("/locations/"), Segment::SingleWildcard, Segment::Literal("/functions/"), Segment::SingleWildcard])?,
            );
        ...
        .ok_or_else(|| {
            let mut paths = Vec::new();
            {
                let builder = PathMismatchBuilder::default();
                let builder = builder.maybe_add(
                    Some(&req).and_then(|m| m.function.as_ref()).map(|m| &m.name).map(|s| s.as_str()),
                    &[Segment::Literal("projects/"), Segment::SingleWildcard, Segment::Literal("/locations/"), Segment::SingleWildcard, Segment::Literal("/functions/"), Segment::SingleWildcard],
                    "function.name",
                    "projects/*/locations/*/functions/*");
                paths.push(builder.build());
    ...
    async fn set_iam_policy(
        ...
        .or_else(|| {
            let path = format!(
                "/v2/{}:setIamPolicy",
                try_match(Some(&req).map(|m| &m.resource).map(|s| s.as_str()), &[Segment::Literal("projects/"), Segment::SingleWildcard, Segment::Literal("/locations/"), Segment::SingleWildcard, Segment::Literal("/functions/"), Segment::SingleWildcard])?,
            );
