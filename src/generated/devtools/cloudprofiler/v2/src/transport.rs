...
impl super::stub::ProfilerService for ProfilerService {
    async fn create_profile(
        ...
        let options = {
            ...
            if let Some(rn) = resource_name {
                let full_resource_name = format!("//cloudprofiler.googleapis.com/{}", rn);
    ...
    async fn create_offline_profile(
        ...
        .or_else(|| {
            ...
            let path_template = "/v2/{parent}/profiles:createOffline";
    ...
    async fn update_profile(
        ...
        .or_else(|| {
            let path = format!(
                "/v2/{}",
                try_match(Some(&req).and_then(|m| m.profile.as_ref()).map(|m| &m.name).map(|s| s.as_str()), &[Segment::Literal("projects/"), Segment::SingleWildcard, Segment::Literal("/profiles/"), Segment::SingleWildcard])?,
            );
            let path_template = "/v2/{profile.name}";
        ...
        .ok_or_else(|| {
            let mut paths = Vec::new();
            {
                let builder = PathMismatchBuilder::default();
                let builder = builder.maybe_add(
                    Some(&req).and_then(|m| m.profile.as_ref()).map(|m| &m.name).map(|s| s.as_str()),
                    &[Segment::Literal("projects/"), Segment::SingleWildcard, Segment::Literal("/profiles/"), Segment::SingleWildcard],
                    "profile.name",
                    "projects/*/profiles/*");
                paths.push(builder.build());
            }
